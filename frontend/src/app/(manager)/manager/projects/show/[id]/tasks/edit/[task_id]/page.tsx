...
export default function TasksPage() {
    ...
    useEffect(() => {
        axios
            .get(`/api/projects/${params.id}/tasks/${params.task_id}`, {
                headers: {
            ...
            .then((res) => {
                setStateData({
                    task_date: res.data.data.task_date,
    ...
    const onSave = () => {
        setIsLoading(true);
        axios
            .patch(`/api/projects/${params.id}/tasks/${params.task_id}`, data, {
                headers: {
                    Authorization: `Bearer ${Cookies.get('token')}`,
                },
            })
            .then((res) => {
                ...
                router.push(`/manager/projects/show/${params.id}`);
            })
            .catch((err) => {
    ...
    return (
        <Page maxWidth="sm">
            <TitleBar title="Edit Task" navigation={`/manager/projects/show/${params.id}`} />
            <BlockStack>
                <Card>
                    <CardContent>
                        <BlockStack>
                            <BlockStack className="gap-2">
                                <Label>Tanggal</Label>
                                <DatePicker onDateChange={(date?: string) => setData('task_date', date || '')} defaultValue={data.task_date} />
                            ...
                            <BlockStack className="gap-2">
                                ...
                                <Select onValueChange={(value) => setData('hours', value)} value={data.hours.toString()}>
